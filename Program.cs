...
namespace StudentManagement
{
    ...
    class Student
    {
        ...
        // События класса Student
        public event StudentEventHandler? OversleepPair;
        public event StudentEventHandler? ReceiveAutomatically;
        public event StudentEventHandler? PassExam;
        ...
        public void AddMark(int value)
        {
            ...
            marks.AddLast(value);
            ResetRating();
            OnReceiveAutomatically(new StudentEventArgs($"Student {name} received a mark of {value}."));
        }
        ...
        public void AddExam(int value)
        {
            ...
            exams.AddLast(value);
            ResetRating();
            OnPassExam(new StudentEventArgs($"Student {name} passed an exam with a mark of {value}."));
        }
        ...
        // Метод для вызова события OversleepPair
        protected virtual void OnOversleepPair(StudentEventArgs e)
        {
            if (OversleepPair != null)
            {
                OversleepPair(this, e);
            }
        }

        // Метод для вызова события ReceiveAutomatically
        protected virtual void OnReceiveAutomatically(StudentEventArgs e)
        {
            if (ReceiveAutomatically != null)
            {
                ReceiveAutomatically(this, e);
            }
        }

        // Метод для вызова события PassExam
        protected virtual void OnPassExam(StudentEventArgs e)
        {
            if (PassExam != null)
            {
                PassExam(this, e);
    ...
    class Group
    {
        ...
        // События класса Group
        public event GroupEventHandler? StudentTransferred;
        public event GroupEventHandler? StudentExcluded;
        ...
        public void TransferStudent(Group anotherGroup, Student student)
        {
            if (students.Remove(student))
            {
                ...
                OnStudentTransferred(new GroupEventArgs($"Student {student.GetName()} {student.GetSurname()} has been transferred to {anotherGroup.GetGroupName()}."));
            }
            else
            {
        ...
        public void ExcludeWorstStudent()
        {
            ...
            OnStudentExcluded(new GroupEventArgs($"Student {worstStudent.GetName()} {worstStudent.GetSurname()} has been excluded."));
        }
        ...
        // Метод для вызова события StudentTransferred
        protected virtual void OnStudentTransferred(GroupEventArgs e)
        {
            if (StudentTransferred != null)
            {
                StudentTransferred(this, e);
            }
        }

        // Метод для вызова события StudentExcluded
        protected virtual void OnStudentExcluded(GroupEventArgs e)
        {
            if (StudentExcluded != null)
            {
                StudentExcluded(this, e);
            }
        }
    }
    ...
    class Program
    {
        static void Main(string[] args)
        {
            try
            {
                ...
                student1.ReceiveAutomatically += StudentEventHandler;
                student1.PassExam += StudentEventHandler;
                student2.ReceiveAutomatically += StudentEventHandler;
                student2.PassExam += StudentEventHandler;

                // Подписка на события группы
                group1.StudentTransferred += GroupEventHandler;
                group1.StudentExcluded += GroupEventHandler;
        ...
        // Обработчик событий студента
        static void StudentEventHandler(object sender, StudentEventArgs e)
        {
            Console.WriteLine($"Student Event: {e.Message}");
        }

        // Обработчик событий группы
        static void GroupEventHandler(object sender, GroupEventArgs e)
        {
            Console.WriteLine($"Group Event: {e.Message}");
        }
